...
public class JWTAuthorizationFiler extends OncePerRequestFilter {
	...
	//cette methode s execute a chaque fois que l user envoit une requete dans le but de savoir si l user a le droit d accéder ou non 
	@Override
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        //preciser les entetes qui ont le droit de lire le contenu de l entete envoyé
        ...
        //pas la peine de chercher le jwt si on voudrait s authentifier
        ...
        {// cest ici ou on commence a analyser le jwt
            String jwtToken = request.getHeader(SecurityParams.JWT_HEADER_NAME);//on recupere le nom de jwt header qui se trouve dans la tete de la requete et  qui doit correspondre au header name qu on a preciser dans le securityparam : autorization
            //ce request genere une exception en cas d erreur
            System.out.println("Token="+jwtToken);
            //s il est null ou ne commence pas par un prefix
            if (jwtToken == null || !jwtToken.startsWith(SecurityParams.HEADER_PREFIX)) {
                filterChain.doFilter(request, response);//je donne rien a springsecurity et je lui demande de passer au filtre suivant
                return;
            }
          ...
          //je donne rien a springsecurity et je lui demande de passer au filtre suivant
            filterChain.doFilter(request, response);
        }
